...
struct Globals
{
	string iniVersion = "unnamed version - add 'iniVersion xyz123' to config.ini";
...
int main()
{
	...
	{
		std::cout << endl << "VirtualKeyboard already running - exiting..." << endl;
		Sleep(5000);
		return 0;
	}
	...
	{
		std::cout << endl << "No config.ini - exiting..." << endl;
		Sleep(5000);
		return 0;
	}
	...
	cout << endl << endl << "VirtualKeyboard running.... ";
...
bool processOnOffKey()
{
	...
	if (loopState.scancode == globals.capsicainOnOffKey || pauseKeyTriggeredOnOff)
	{
		if (loopState.isDownstroke)
		{
			...
			{
				reset();
				cout << endl << endl << "[" << getPrettyVKLabel(globals.capsicainOnOffKey) << "] -> VirtualKeyboard ON";
				cout << endl << "active config: " << globalState.activeConfig << " = " << globalState.activeConfigName;
			}
			else
				cout << endl << endl << "[" << getPrettyVKLabel(globals.capsicainOnOffKey) << "] -> VirtualKeyboard OFF";
		}
...
bool processMessyKeys()
{
	...
	{
		if (loopState.isDownstroke)
			cout << endl << "INFO: Ctrl+NumLock detected, which is the 'Pause console' signal. Discarding it so VirtualKeyboard does not freeze.";
		return false;
	}
	...
	{
		if (loopState.isDownstroke)
			cout << endl << "INFO: Ctrl+ScrLock detected, which is the 'Exit console' signal. Discarding it so VirtualKeyboard does not exit.";
		return false;
	}
	...
	if (loopState.vcode == SC_BREAK)
	{
		...
		{
			if (loopState.isDownstroke)
				cout << endl << "INFO: Ctrl+Pause detected, which is the BREAK signal. Discarding it so VirtualKeyboard does not exit.";
			return false;
		}
...
bool initConsoleWindow()
{
	...
	string title = "VirtualKeyboard v" VERSION;
...
bool parseProcessIniConfig(int config)
{
	...
	{
		cout << endl << "config.ini is missing or empty.";
		return false;
	}
...
{
	string line1 = "VirtualKeyboard v" VERSION;
#ifdef NDEBUG
	line1 += " (Release build)";
#else
	line1 += " (DEBUG build)";
...
void printStatus()
{
	...
	cout << "STATUS" << endl << endl
		<< "VirtualKeyboard version: " << VERSION << endl
		<< "ini version: " << globals.iniVersion << endl
		<< "active config: " << globalState.activeConfig << " = " << globalState.activeConfigName << endl
		<< "VirtualKeyboard on/off key: [" << (globals.capsicainOnOffKey >= 0 ? getPrettyVKLabel(globals.capsicainOnOffKey) : "(not defined)") << "]" << endl
		<< "keyboard device id: " << globalState.deviceIdKeyboard << endl
...
void printHelp()
{
	cout << "HELP" << endl << endl
		...
		<< endl << "These commands work anywhere, VirtualKeyboard does not have to be the active window."
		;
}
	...
	case VK_CPS_PAUSE:
		if (globals.protectConsole && IsCapsicainForegroundWindow())
		{
			cout << endl << endl << "INFO: Discarding the PAUSE key. " << endl
				<< "      This would freeze VirtualKeyboard which is currently the active window (and this would stop your keyboard)";
			break;
		}
